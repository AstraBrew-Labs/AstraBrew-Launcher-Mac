...
fn render_basic_settings(ui: &mut egui::Ui, state: &mut SettingsState, lang_state: &Language) {
    ...
    // 代理端口
    ui.label(egui::RichText::new(lang::t("rp_port", lang_state)).size(13.0).strong());
    ui.label(
        egui::RichText::new(lang::t("rp_port_desc", lang_state))
            .color(egui::Color32::GRAY)
            .size(11.0),
    );
    ui.add_space(4.0);
    ui.add(
        egui::TextEdit::singleline(&mut state.reverse_proxy_port)
            .desired_width(120.0),
    );
    ui.add_space(14.0);

    // 目标地址
    ui.label(egui::RichText::new(lang::t("rp_target", lang_state)).size(13.0).strong());
    ui.label(
        egui::RichText::new(lang::t("rp_target_desc", lang_state))
            .color(egui::Color32::GRAY)
            .size(11.0),
    );
    ui.add_space(4.0);
    ui.add(
        egui::TextEdit::singleline(&mut state.reverse_proxy_target)
            .hint_text(lang::t("rp_target_hint", lang_state))
            .desired_width(f32::INFINITY),
    );
}
...
fn render_ssl_settings(ui: &mut egui::Ui, state: &mut SettingsState, lang_state: &Language) {
    ...
    let text_height = 220.0;
    let half_width = 300.0;
